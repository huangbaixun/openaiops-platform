...
# (the only package authorized to bypass MustTenantScope). _test.go files are
...
if [ -n "$BAD_ADMIN" ]; then
    echo "FAIL: chquery.NewAdminConn constructed outside internal/topoengine/:" >&2
    echo "$BAD_ADMIN" >&2
    echo "" >&2
    echo "Violation: AdminConn bypasses MustTenantScope. Only internal/topoengine/" >&2
    echo "may construct it. See SLICE-3 spec §2." >&2
    exit 1
fi

echo "lint-no-bare-ch: OK ($SCAN_DIRS clean; AdminConn confined to internal/topoengine/)"
